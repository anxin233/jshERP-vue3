    ...
    <div class="table-page-search-wrapper">
      <el-form :inline="true" @submit.prevent="searchQuery" class="search-form">
        <el-row :gutter="24">
          ...
          <el-col :md="6" :sm="24">
            <el-form-item>
              <span style="font-weight: bold">实际零售金额：{{ realityPriceTotal }}</span>
            </el-form-item>
          </el-col>
        </el-row>
...
defineOptions({ name: 'RetailOutReport' })
...
const defColumns: ListColumn[] = [
  ...
  { title: '零售数量', dataIndex: 'outSum', width: 80, sorter: true },
  { title: '零售金额', dataIndex: 'outSumPrice', width: 80, sorter: true },
  { title: '退货数量', dataIndex: 'inSum', width: 80, sorter: true },
  { title: '退货金额', dataIndex: 'inSumPrice', width: 80, sorter: true },
  { title: '实际零售金额', dataIndex: 'outInSumPrice', width: 100, sorter: true },
]
...
const {
  ...
  url: { list: '/depotItem/retailOut' },
  defColumns,
  defDataIndex,
  pageName: 'retailOutReport',
  urlPath: '/report/retail_out',
  pageSize: 11,
...
async function loadReportData(arg?: number) {
  ...
  try {
    ...
    const res = await getAction('/depotItem/retailOut', params)
    if (res.code === 200) {
    ...
    console.error('[RetailOutReport] loadData error:', err)
  } finally {
    loading.value = false
  }
}
...
// ==================== 会员/仓库/机构/类别 ====================
function initRetail() {
  getAction('/supplier/findBySelect_retail', { limit: 1 }).then((res: any) => {
    if (res) retailList.value = res
  })
}
...
function handleSearchRetail(value: string) {
  ...
  setTimeFlag.value = setTimeout(() => {
    getAction('/supplier/findBySelect_retail', { key: value, limit: 1 }).then((res: any) => {
      if (res) retailList.value = res
    })
  }, 500)
}
...
function exportExcel() {
  ...
  const head = '条码,名称,规格,型号,颜色,品牌,制造商,' + mpStr + ',单位,零售数量,零售金额,退货数量,退货金额,实际零售金额'
  ...
  handleExportXlsPost('零售统计', '零售统计', head, tip, list)
}
