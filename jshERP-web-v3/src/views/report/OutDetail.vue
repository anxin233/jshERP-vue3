...
async function customLoadData(arg?: number) {
  ...
  try {
    ...
    console.error('[OutDetail] loadData error:', err)
  } finally {
    loading.value = false
  }
}
...
function exportExcel() {
  const list: any[] = []
  const head = '单据编号,条码,名称,规格,型号,颜色,品牌,制造商,单位,多属性,数量,单价,金额,税率(%),税额,往来单位,仓库,出库日期,备注'
  ...
  handleExportXlsPost('出库明细', '出库明细', head, tip, list)
}
