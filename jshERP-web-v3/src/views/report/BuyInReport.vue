    ...
    <div class="table-page-search-wrapper">
      <el-form :inline="true" @submit.prevent="searchQuery" class="search-form">
        <el-row :gutter="24">
          ...
          <el-col :md="6" :sm="24">
            <el-form-item>
              <span style="font-weight: bold">实际采购金额：{{ realityPriceTotal }}</span>
            </el-form-item>
          </el-col>
        </el-row>
...
defineOptions({ name: 'BuyInReport' })
...
const defColumns: ListColumn[] = [
  ...
  { title: '采购数量', dataIndex: 'inSum', width: 80, sorter: true },
  { title: '采购金额', dataIndex: 'inSumPrice', width: 80, sorter: true },
  { title: '退货数量', dataIndex: 'outSum', width: 80, sorter: true },
  { title: '退货金额', dataIndex: 'outSumPrice', width: 80, sorter: true },
  { title: '实际采购金额', dataIndex: 'inOutSumPrice', width: 100, sorter: true },
]

const defDataIndex = [
  'rowIndex', 'barCode', 'materialName', 'materialStandard', 'materialModel',
  'materialUnit', 'inSum', 'inSumPrice', 'outSum', 'outSumPrice', 'inOutSumPrice',
]
...
const {
  ...
  url: { list: '/depotItem/buyIn' },
  defColumns,
  defDataIndex,
  pageName: 'buyInReport',
  urlPath: '/report/buy_in',
  pageSize: 11,
...
async function loadReportData(arg?: number) {
  ...
  try {
    ...
    const res = await getAction('/depotItem/buyIn', params)
    if (res.code === 200) {
    ...
    console.error('[BuyInReport] loadData error:', err)
  } finally {
    loading.value = false
  }
}
...
function initSupplier() {
  getAction('/supplier/findBySelect_sup', { limit: 1 }).then((res: any) => {
...
function handleSearchSupplier(value: string) {
  ...
  setTimeFlag.value = setTimeout(() => {
    getAction('/supplier/findBySelect_sup', { key: value, limit: 1 }).then((res: any) => {
      if (res) supList.value = res
    })
  }, 500)
}
...
function exportExcel() {
  ...
  const head = '条码,名称,规格,型号,颜色,品牌,制造商,' + mpStr + ',单位,进货数量,进货金额,退货数量,退货金额,实际采购金额'
  for (let i = 0; i < dataSource.value.length; i++) {
    ...
    list.push([
      ...
      ds.materialUnit, ds.inSum, ds.inSumPrice, ds.outSum, ds.outSumPrice, ds.inOutSumPrice,
    ])
  }
  const tip = '单据日期：' + queryParam.beginTime + '~' + queryParam.endTime
  handleExportXlsPost('进货统计', '进货统计', head, tip, list)
}
