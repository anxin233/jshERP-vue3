...
const defColumns: ListColumn[] = [
  ...
  { title: '入库数量', dataIndex: 'numSum', sorter: true, width: 120 },
  { title: '入库金额', dataIndex: 'priceSum', sorter: true, width: 120 },
]
...
const {
  ...
  pageSize: 11,
  urlPath: '/report/in_material_count',
...
async function customLoadData(arg?: number) {
  ...
  try {
    ...
    console.error('[InMaterialCount] loadData error:', err)
  } finally {
    loading.value = false
  }
}
...
function exportExcel() {
  const list: any[] = []
  const head = '条码,名称,规格,型号,颜色,品牌,制造商,类别,单位,入库数量,入库金额'
  ...
  handleExportXlsPost('入库汇总', '入库汇总', head, tip, list)
}
