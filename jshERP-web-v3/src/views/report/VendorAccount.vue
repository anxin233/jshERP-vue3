    ...
    >
      ...
      <template #headerCell="{ column }">
        <template v-if="column.dataIndex === 'allNeed'">
          期末应付
          <el-tooltip content="期末应付=期初应付+本期欠款-本期付款" placement="top">
...
defineOptions({ name: 'VendorAccount' })
...
const defColumns: ListColumn[] = [
  ...
  { title: '期初应付', dataIndex: 'preNeed', width: 80, sorter: true },
  { title: '本期欠款', dataIndex: 'debtMoney', width: 80, sorter: true },
  { title: '本期付款', dataIndex: 'backMoney', width: 80, sorter: true },
  { title: '期末应付', dataIndex: 'allNeed', width: 80, sorter: true },
]
...
async function loadReportData(arg?: number) {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      firstTotal.value = '期初应付：' + (res.data.firstMoney || 0) + '，'
      lastTotal.value = '期末应付：' + (res.data.lastMoney || 0)
    ...
    console.error('[VendorAccount] loadData error:', err)
  } finally {
    loading.value = false
  }
}
...
function exportExcel() {
  const list: any[][] = []
  const head = '供应商,联系人,手机号码,联系电话,电子邮箱,期初应付,本期欠款,本期付款,期末应付'
  ...
  handleExportXlsPost('供应商对账', '供应商对账', head, tip, list)
}
