  ...
  <div class="bill-list-page">
    <el-card shadow="never">
      ...
      >
        <!-- 操作列 -->
        <template #action="{ row }">
          <el-button link type="primary" size="small" @click="myHandleDetail(row, '零售退货入库', prefixNo)">查看</el-button>
        ...
        <!-- 单据编号渲染：转单标记 -->
        <template #number="{ row }">
          {{ row.linkNumber ? row.number + '[转]' : row.number }}
        </template>

        <!-- 付款金额 -->
        <template #getAmount="{ row }">
          {{ row.backAmount ? row.changeAmount + row.backAmount : row.changeAmount }}
        </template>
      ...
      <!-- ==================== 弹窗区域 ==================== -->
      <RetailBackModal ref="modalFormRef" @ok="modalFormOk" @close="modalFormClose" />
...
defineOptions({ name: 'RetailBackList' })
