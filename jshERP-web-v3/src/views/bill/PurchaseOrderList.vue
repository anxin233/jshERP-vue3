  ...
  <div class="bill-list-page">
    <el-card shadow="never">
            ...
            <el-row :gutter="24">
              ...
              <el-col :md="6" :sm="24">
                <el-form-item label="关联请购单">
                  <el-input v-model="queryParam.linkApply" placeholder="请输入关联请购单" clearable />
                </el-form-item>
              </el-col>
              <el-col v-if="purchaseBySaleFlag" :md="6" :sm="24">
              ...
              <el-col :md="6" :sm="24">
                <el-form-item label="单据状态">
                  <el-select v-model="queryParam.status" placeholder="请选择单据状态" clearable style="width: 100%">
                    ...
                    <el-option label="部分采购" value="3" />
                    <el-option label="完成采购" value="2" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :md="6" :sm="24">
      ...
      <div class="table-operator" style="margin-top: 5px; margin-bottom: 10px">
        ...
        <el-button
          v-if="quickBtn.purchaseIn && quickBtn.purchaseIn.indexOf(1) > -1 && btnEnableList.indexOf(1) > -1"
          :icon="Share"
          @click="transferBill('转采购入库', quickBtn.purchaseIn)"
        >
          转采购入库
        </el-button>
        <el-tooltip content="可将状态是部分采购的单据强制完成" placement="top">
          <el-button
        ...
        <el-popover trigger="click" placement="right" :width="520">
          ...
          <el-checkbox-group v-model="settingDataIndex" @change="onColChange">
            <el-row>
              <template v-for="item in defColumns" :key="item.dataIndex">
                <el-col :span="8" v-if="purchaseBySaleFlag || item.dataIndex !== 'linkNumber'">
        ...
        >
          <template #content>
            <div style="max-width: 300px">
              采购订单不涉及付款金额，采购订单可以转采购入库单，但需要先对采购订单进行审核。<br />
      ...
      >
        <!-- 操作列 -->
        <template #action="{ row }">
          <el-button link type="primary" size="small" @click="myHandleDetail(row, '采购订单', prefixNo)">查看</el-button>
        ...
        <template #customRenderStatus="{ row }">
          ...
          <el-tag v-else-if="row?.status == '2'" size="small">完成采购</el-tag>
          <el-tag v-else-if="row?.status == '3'" type="primary" size="small">部分采购</el-tag>
          <el-tag v-else-if="row?.status == '9'" type="warning" size="small">审核中</el-tag>
        </template>
      </ProTable>

      <!-- ==================== 弹窗区域 ==================== -->
      <PurchaseOrderModal ref="modalFormRef" @ok="modalFormOk" @close="modalFormClose" />
      <!-- 转单弹窗：采购入库 -->
      <!-- <PurchaseInModal ref="transferModalFormRef" @ok="modalFormOk" @close="modalFormClose" /> -->
...
// import PurchaseInModal from './components/PurchaseInModal.vue'
...
const defDataIndex = [
  ...
  'materialCount', 'totalPrice', 'totalTaxLastMoney', 'changeAmount', 'status',
]
