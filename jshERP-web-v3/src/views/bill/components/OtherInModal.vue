<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('其它入库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('其它入库', $refs.modalPrint)">
        三联打印
      </el-button>
...
defineOptions({ name: 'OtherInModal' })
...
const materialTable = reactive({
  ...
  columns: [
    ...
    { title: '批号', key: 'batchNumber', width: '7%', type: 'input' as const },
    { title: '有效期', key: 'expirationDate', width: '7%', type: 'date' as const },
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[OtherInModal] handleOk error:', e)
...
function onSearchLinkNumber() {
  linkBillListRef.value?.show('入库', '采购,销售退货', '1,3')
  if (linkBillListRef.value) {
    linkBillListRef.value.title = '请选择采购入库或销售退货'
  }
}
