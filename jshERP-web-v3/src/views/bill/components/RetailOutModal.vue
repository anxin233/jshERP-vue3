<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('零售出库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('零售出库', $refs.modalPrint)">
        三联打印
      </el-button>
...
defineOptions({ name: 'RetailOutModal' })
...
const payTypeList = ref<{ value: string; text: string }[]>([])
...
const materialTable = reactive({
  ...
  columns: [
    ...
    { title: '有效期', key: 'expirationDate', width: '9%', type: 'input' as const, readonly: true },
...
function initPayTypeList() {
  payTypeList.value = [{ value: '现付', text: '现付' }]
}

/** 自定义初始化结算账户（零售出库的特有逻辑） */
...
/** 选择会员的触发事件 */
function onChangeOrgan(value: any) {
  getAction('/supplier/info', { id: value }).then((res: any) => {
    if (res && res.code === 200) {
      payTypeList.value = []
      const info = res.data.info
      if (info.advanceIn) {
        payTypeList.value.push({ value: '预付款', text: '预付款（' + info.advanceIn + '）' })
        payTypeList.value.push({ value: '现付', text: '现付' })
        nextTick(() => {
          formModel.payType = '预付款'
        })
      } else {
        payTypeList.value.push({ value: '现付', text: '现付' })
        nextTick(() => {
          formModel.payType = '现付'
        })
      }
    }
  })
...
/** 自动改变价格（零售出库覆盖：只设置 changeAmount/getAmount/backAmount） */
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[RetailOutModal] handleOk error:', e)
