<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('调拨出库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('调拨出库', $refs.modalPrint)">
        三联打印
      </el-button>
...
defineOptions({ name: 'AllocationOutModal' })
...
const materialTable = reactive({
  ...
  columns: [
    ...
    {
      title: '调入仓库', key: 'anotherDepotId', width: '8%', type: 'select' as const,
      placeholder: '请选择调入仓库', options: [] as any[],
      allowSearch: true,
    },
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[AllocationOutModal] handleOk error:', e)
