<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('采购订单', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('采购订单', $refs.modalPrint)">
        三联打印
      </el-button>
    ...
    >
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="关联请购单" prop="linkApply">
            <el-input
              v-model="formModel.linkApply"
              placeholder="请选择关联请购单"
              readonly
            >
              <template #append>
                <el-button :icon="Search" @click="onSearchLinkApply" />
              </template>
            </el-input>
          </el-form-item>
        </el-col>
        <el-col v-if="purchaseBySaleFlag" :lg="6" :md="12" :sm="24">
      ...
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="支付订金">
            <el-input v-model.trim="formModel.changeAmount" placeholder="请输入支付订金" @change="onChangeChangeAmount" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="12" :sm="24" />
        <el-col :lg="6" :md="12" :sm="24" />
  ...
  if (action.value === 'add') {
    ...
    nextTick(() => {
      if (transferParam.value && transferParam.value.number) {
        ...
        linkBillListOk(tp.list, tp.number, tp.organId)
      }
    })
  } else {
    // 编辑/查看
    ...
    if (formModel.accountId == null && formModel.accountIdList) {
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[PurchaseOrderModal] handleOk error:', e)
...
function handleHistoryBillList() {
  ...
  historyBillListRef.value?.show('其它', '采购订单', '供应商', organId)
}

/** 搜索关联销售订单（以销定购） */
function onSearchLinkNumber() {
  linkBillListRef.value?.purchaseShow('其它', '销售订单', '客户', '1,3', '0,3')
  if (linkBillListRef.value) {
    linkBillListRef.value.title = '请选择销售订单'
  }
}

/** 搜索关联请购单 */
function onSearchLinkApply() {
  linkBillListRef.value?.purchaseShow('其它', '请购单', '客户', '1,3')
  if (linkBillListRef.value) {
    linkBillListRef.value.title = '请选择请购单'
  }
}
  ...
  if (selectBillDetailRows && selectBillDetailRows.length > 0) {
    ...
    for (let j = 0; j < selectBillDetailRows.length; j++) {
      ...
      if (info.preNumber) {
        ...
        info.allPrice = (info.operNumber * info.unitPrice).toFixed(2) as unknown as number - 0
        info.taxRate = 0
        info.taxMoney = 0
    ...
    // 根据单号查询单据类型
    findBillDetailByNumber({ number: linkNumber }).then((res: any) => {
      if (res.code === 200) {
        if (res.data && res.data.subType === '请购单') {
          // 关联请购单
          nextTick(() => {
            formModel.linkApply = linkNumber
          })
        } else {
          nextTick(() => {
    ...
    // 给优惠后金额重新赋值
    discountLastMoney = discountLastMoney || 0
    nextTick(() => {
      formModel.discountLastMoney = discountLastMoney.toFixed(2)
