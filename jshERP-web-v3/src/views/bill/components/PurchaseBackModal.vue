<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('采购退货出库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('采购退货出库', $refs.modalPrint)">
        三联打印
      </el-button>
...
import {
  Plus, Refresh, Search, FolderOpened,
...
defineOptions({ name: 'PurchaseBackModal' })
    ...
    nextTick(() => {
      if (transferParam.value && transferParam.value.number) {
        ...
        linkBillListOk(
          ...
          0, tp.remark, defaultDepotId.value, tp.accountId,
        )
      }
    })
  } else {
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[PurchaseBackModal] handleOk error:', e)
...
function onSearchLinkNumber() {
  linkBillListRef.value?.show('入库', '采购', '供应商', '1,2,3')
  if (linkBillListRef.value) {
    linkBillListRef.value.title = '请选择采购入库'
  }
}
