<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('拆卸单', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('拆卸单', $refs.modalPrint)">
        三联打印
      </el-button>
...
defineOptions({ name: 'DisassembleModal' })
...
const materialTable = reactive({
  ...
  columns: [
    ...
    { title: '多属性', key: 'sku', width: '4%', type: FormTypes.normal },
    {
      ...
      validateRules: [{ required: true, message: '数量不能为空' }],
    },
    { title: '单价', key: 'unitPrice', width: '5%', type: 'inputNumber' as const },
    { title: '金额', key: 'allPrice', width: '5%', type: 'inputNumber' as const },
...
/** 新增行时的回调 - 拆卸单特有逻辑 */
function onAddedRow(event: { row: any; target: any }) {
  // 先调用通用 onAdded
  onAdded(event)
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[DisassembleModal] handleOk error:', e)
