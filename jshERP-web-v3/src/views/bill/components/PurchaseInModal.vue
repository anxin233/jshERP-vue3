<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('采购入库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('采购入库', $refs.modalPrint)">
        三联打印
      </el-button>
    ...
    >
      <el-row :gutter="24">
          ...
          <el-form-item label="供应商" prop="organId">
            ...
            >
              ...
              <template #footer>
                ...
                <div class="dropdown-btn" @click="initSupplier(0)">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="关联订单" prop="linkNumber">
            <el-input
      ...
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="付款优惠">
            <el-input v-model.trim="formModel.discountMoney" placeholder="请输入付款优惠" @change="onChangeDiscountMoney" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="12" :sm="24">
      ...
      <el-row :gutter="24">
        ...
        <el-col v-if="depositStatus" :lg="6" :md="12" :sm="24">
          <el-form-item label="扣除订金">
            <el-input v-model.trim="formModel.deposit" @change="onChangeDeposit" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="本次付款" prop="changeAmount">
            <el-input v-model.trim="formModel.changeAmount" placeholder="请输入本次付款" @change="onChangeChangeAmount" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="12" :sm="24">
    ...
    <DepotModal ref="depotModalForm" @ok="depotModalFormOk" />
...
import DepotModal from '@/views/system/components/DepotModal.vue'
...
import { getCurrentSystemConfig, findBySelectSup } from '@/api/system'
import { getMpListShort } from '@/utils/index'
import { getStore } from '@/utils/storage'

defineOptions({ name: 'PurchaseInModal' })
    ...
    nextTick(() => {
      if (transferParam.value && transferParam.value.number) {
        ...
        linkBillListOk(
          ...
          tp.deposit, tp.remark, defaultDepotId.value, tp.accountId,
        )
      }
    })
  } else {
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[PurchaseInModal] handleOk error:', e)
...
function handleHistoryBillList() {
  ...
  historyBillListRef.value?.show('入库', '采购', '供应商', organId)
}

/** 搜索关联订单 */
function onSearchLinkNumber() {
  linkBillListRef.value?.show('其它', '采购订单', '供应商', '1,3')
  if (linkBillListRef.value) {
    linkBillListRef.value.title = '请选择采购订单'
  }
}
  ...
  if (selectBillDetailRows && selectBillDetailRows.length > 0) {
    ...
    for (let j = 0; j < selectBillDetailRows.length; j++) {
      ...
      if (info.finishNumber > 0) {
        ...
        info.taxMoney = ((info.allPrice * taxRate) / 100).toFixed(2) as unknown as number - 0
        info.taxLastMoney = (info.allPrice + info.taxMoney).toFixed(2) as unknown as number - 0
      }
      info.linkId = info.id
    ...
    if (deposit) {
      depositStatus.value = true
      changeAmount = (discountLastMoney - deposit).toFixed(2) as unknown as number - 0
    }
