<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('销售出库', $refs.modalPrintPro)">
        三联打印-新版
      </el-button>
      <el-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('销售出库', $refs.modalPrint)">
        三联打印
      </el-button>
    ...
    >
      ...
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="本次收款" prop="changeAmount">
            <el-input v-model.trim="formModel.changeAmount" placeholder="请输入本次收款" @change="onChangeChangeAmount" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="12" :sm="24">
...
defineOptions({ name: 'SaleOutModal' })
    ...
    nextTick(() => {
      if (transferParam.value && transferParam.value.number) {
        ...
        linkBillListOk(
          ...
          tp.deposit, tp.remark, defaultDepotId.value, tp.accountId, tp.salesMan,
        )
      }
    })
  } else {
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[SaleOutModal] handleOk error:', e)
...
function handleHistoryBillList() {
  ...
  historyBillListRef.value?.show('出库', '销售', '客户', organId)
}

/** 搜索关联订单 */
function onSearchLinkNumber() {
  linkBillListRef.value?.show('其它', '销售订单', '客户', '1,3')
