  ...
  <div class="bill-list-page">
    <el-card shadow="never">
      ...
      <div class="table-operator" style="margin-top: 5px; margin-bottom: 10px">
        ...
        <el-button
          v-if="quickBtn.retailBack.indexOf('1') > -1 && btnEnableList.indexOf(1) > -1"
          :icon="Share"
          @click="transferBill('转零售退货', quickBtn.retailBack)"
        >
          转零售退货
        </el-button>
      ...
      >
        <!-- 操作列 -->
        <template #action="{ row }">
          <el-button link type="primary" size="small" @click="myHandleDetail(row, '零售出库', prefixNo)">查看</el-button>
        ...
        <!-- 单据编号渲染：退货标记 -->
        <template #number="{ row }">
          {{ row?.hasBackFlag ? row.number + '[退]' : row?.number || '' }}
        </template>

        <!-- 收款金额 -->
        <template #getAmount="{ row }">
          {{ row?.backAmount ? row.changeAmount + row.backAmount : row?.changeAmount || 0 }}
        </template>
      ...
      <!-- ==================== 弹窗区域 ==================== -->
      <RetailOutModal ref="modalFormRef" @ok="modalFormOk" @close="modalFormClose" />
      <!-- 转单弹窗：零售退货 -->
      <RetailBackModal ref="transferModalFormRef" @ok="modalFormOk" @close="modalFormClose" />
...
import {
  Plus, Delete, Share, Check, Close,
  Download, Setting, QuestionFilled, ArrowUp, ArrowDown,
} from '@element-plus/icons-vue'
import ProTable from '@/components/table/ProTable.vue'
import RetailOutModal from './components/RetailOutModal.vue'
import RetailBackModal from './components/RetailBackModal.vue'
...
defineOptions({ name: 'RetailOutList' })
...
const defDataIndex = [
  ...
  'materialCount', 'totalPrice', 'getAmount', 'backAmount', 'status',
]
