  ...
  <div class="bill-list-page">
    <el-card shadow="never">
              ...
              <el-col :md="6" :sm="24">
                <el-form-item label="单据状态">
                  <el-select v-model="queryParam.status" placeholder="请选择单据状态" clearable style="width: 100%">
                    ...
                    <el-option label="部分入库" value="3" />
                    <el-option label="完成入库" value="2" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :md="6" :sm="24">
      ...
      <div class="table-operator" style="margin-top: 5px; margin-bottom: 10px">
        ...
        <el-button
          v-if="quickBtn.purchaseBack && quickBtn.purchaseBack.indexOf(1) > -1 && btnEnableList.indexOf(1) > -1"
          :icon="Share"
          @click="transferBill('转采购退货', quickBtn.purchaseBack)"
        >
          转采购退货
        </el-button>
        <el-tooltip content="可将状态是部分入库的单据强制完成" placement="top">
          <el-button
        ...
        >
          <template #content>
            <div style="max-width: 300px">
              采购入库单可以由采购订单转过来，也可以单独创建。<br />
              采购入库单据中的仓库列表只显示当前用户有权限的仓库。采购入库单可以使用多账户付款。<br />
      ...
      >
        <!-- 操作列 -->
        <template #action="{ row }">
          <el-button link type="primary" size="small" @click="myHandleDetail(row, '采购入库', prefixNo)">查看</el-button>
        ...
        <template #customRenderDebt="{ row }">
          <el-tooltip v-if="row.debt > 0 && row.debt > row.lastDebt" content="有付款单">
            <span style="color: green">{{ row.debt }}</span>
          </el-tooltip>
          <el-tooltip v-else-if="row.debt > 0 && row.debt === row.lastDebt" content="暂未付款">
        ...
        <template #customRenderStatus="{ row }">
          ...
          <el-tag v-else-if="row?.status == '2'" size="small">完成入库</el-tag>
          <el-tag v-else-if="row?.status == '3'" type="primary" size="small">部分入库</el-tag>
          <el-tag v-else-if="row?.status == '9'" type="warning" size="small">审核中</el-tag>
        </template>
      </ProTable>

      <!-- ==================== 弹窗区域 ==================== -->
      <PurchaseInModal ref="modalFormRef" @ok="modalFormOk" @close="modalFormClose" />
      <!-- 转单弹窗：采购退货 -->
      <!-- <PurchaseBackModal ref="transferModalFormRef" @ok="modalFormOk" @close="modalFormClose" /> -->
...
// import PurchaseBackModal from './components/PurchaseBackModal.vue'
...
defineOptions({ name: 'PurchaseInList' })
