<template>
  ...
  >
    ...
    >
      <el-row :gutter="24">
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="往来单位" prop="organId">
            <el-select
      ...
      <div class="editable-table-section">
        ...
        >
          <template #inOutItemIdFooter>
            <div v-if="quickBtn.inOutItem" class="dropdown-btn" @click="addInOutItem('out', $refs.inOutItemModalForm)">
              <el-icon><Plus /></el-icon> 新增收支项目
            </div>
            <div class="dropdown-btn" @click="initInOutItem('out', accountTable)">
              <el-icon><Refresh /></el-icon> 刷新列表
            </div>
          </template>
        </EditableTable>
      ...
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="合计金额">
            <el-input v-model.trim="formModel.changeAmount" placeholder="请输入支出金额" readonly />
          </el-form-item>
        </el-col>
    ...
    <InOutItemModal ref="inOutItemModalForm" @ok="inOutItemModalFormOk('out')" />
  </el-dialog>
</template>
...
import InOutItemModal from '@/views/system/components/InOutItemModal.vue'
import { useFinancialModal } from '@/composables/useFinancialModal'
import { postAction, getAction } from '@/api/http'

defineOptions({ name: 'ItemOutModal' })
...
// ==================== 表单验证 ====================
const formRules = {
  organId: [{ required: true, message: '请选择往来单位!', trigger: 'change' }],
...
const accountTable = reactive({
  ...
  columns: [
    {
      title: '支出项目', key: 'inOutItemId', width: '20%', type: 'select' as const,
      placeholder: '请选择支出项目', options: [] as any[],
      validateRules: [{ required: true, message: '支出项目不能为空' }],
    },
    {
...
function editAfter() {
  ...
  initOrgan()
  initPerson()
  initInOutItem('out', accountTable)
  initAccount()
  initQuickBtn()
}
...
async function requestSubTableData(urlPath: string, params: any, table: any) {
  ...
  try {
    ...
    console.error('[ItemOutModal] requestSubTableData error:', err)
  } finally {
    table.loading = false
  }
}
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[ItemOutModal] handleOk error:', e)
