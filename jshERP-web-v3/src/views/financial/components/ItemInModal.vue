<template>
  ...
  >
      ...
      <div class="editable-table-section">
        ...
        >
          <template #inOutItemIdFooter>
            <div v-if="quickBtn.inOutItem" class="dropdown-btn" @click="addInOutItem('in', $refs.inOutItemModalForm)">
              <el-icon><Plus /></el-icon> 新增收支项目
            </div>
            <div class="dropdown-btn" @click="initInOutItem('in', accountTable)">
              <el-icon><Refresh /></el-icon> 刷新列表
            </div>
          </template>
        </EditableTable>
      ...
      <el-row :gutter="24">
        ...
        <el-col :lg="6" :md="12" :sm="24">
          <el-form-item label="合计金额">
            <el-input v-model.trim="formModel.changeAmount" placeholder="请输入收入金额" readonly />
          </el-form-item>
        </el-col>
    ...
    <InOutItemModal ref="inOutItemModalForm" @ok="inOutItemModalFormOk('in')" />
  </el-dialog>
</template>
...
defineOptions({ name: 'ItemInModal' })
...
const accountTable = reactive({
  ...
  columns: [
    {
      title: '收入项目', key: 'inOutItemId', width: '20%', type: 'select' as const,
      placeholder: '请选择收入项目', options: [] as any[],
      validateRules: [{ required: true, message: '收入项目不能为空' }],
    },
    {
...
async function requestSubTableData(urlPath: string, params: any, table: any) {
  ...
  try {
    ...
    console.error('[ItemInModal] requestSubTableData error:', err)
  } finally {
    table.loading = false
  }
}
...
async function handleOk() {
  ...
  try {
    ...
    console.error('[ItemInModal] handleOk error:', e)
