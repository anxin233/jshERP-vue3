...
> `docker-compose.external-mysql.yml` + `docker-compose.server.yml` + 本目录 `mysql-external-docker-network.yml`（三份 `-f`），`.env` 与原先一致即可。
...
```bash
cd deploy/docker
docker compose -f docker-compose.external-mysql.yml -f docker-compose.server.yml \
