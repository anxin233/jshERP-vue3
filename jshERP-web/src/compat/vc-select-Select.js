// Fork of ant-design-vue/es/vc-select/Select.js — Vue 3 compat: getOptionsFromChildren for vnode without data
  ...
  methods: {
    ...
    getOptionsFromChildren: function getOptionsFromChildren() {
      ...
      children.forEach(function (child) {
        ...
        var slot = child.data && child.data.slot;
        if (slot === undefined && child.props && child.props.slot !== undefined) {
          slot = child.props.slot;
        }
        if (slot !== undefined) {
          return;
        }
        if (!child.data && !child.__v_isVNode) {
          return;
        }
        if (getSlotOptions(child).isSelectOptGroup) {
          ...
          _this3.getOptionsFromChildren(groupChildren || [], options);
        } else {
          options.push(child);
        }
      });
